...
export const Piza = memo(({name,price,img,id}) => {
    ...
    const addPicaToCart = () => {
            ...
            setCount(count + 1);
            dispatch(addPiza({img,id,name,type: pizzaTypes[avaibleTypes],size : pizzaSizes[avaibleSizes],price,showMessage:false}))
            dispatch(increment())
    };
            ...
            <div className={styles.pizza__column}>
                ...
                <div className={styles.pizza__options}>
                    ...
                    {pizzaTypes.map((item,index)=>{
                        return(
                            <div key={item} onClick={()=>{setAvaibleTypes(index)}} 
                            className={`${styles.type__item__wrapper}`}>
                                <p className={`${styles.pizza__type__item} ${index===avaibleTypes ? styles.pizza__type__item__active : ""}`}>
                                    {item}
                                </p>
                            </div>
                        )
                    ...
                    <div className={styles.pizza__size}>
                        {pizzaSizes.map((item,index)=>{
                            return (
                            <div key={item} onClick={()=>{setAvaibleSizes(index)}}  className={styles.size__item__wrapper}>
                                <div className={`${styles.pizza__size__item} ${index===avaibleSizes ? styles.pizza__size__item__active : ""}`}>
                                    {item}
                                </div>
                            </div>
                            );
                ...
                <div className={styles.price}>
                    <div className={styles.price__money}>
                        <p className={styles.text}>от {price} {'\u20BD'} </p>
                    </div>
                    <div onClick={addPicaToCart} className={`${styles["btn"]} btn`}>
                        ...
                        <div className={styles.btn__count}>{reducedAmount()}</div>
